...
(function () {
   ...
   */
  EventEmitter.prototype._trigger = function (name, data) {
   ...
   */
  EventEmitter.prototype.unbind = function (name, callback) {
  ...
  luajs = new EventEmitter ();
  luajs.EventEmitter = EventEmitter;
...
luajs.Closure.prototype.execute = function (args) {
  ...
  try {
    ...
    if (!(e instanceof luajs.Error) && e.message.substr (0, 7) !== '[luajs]') {
      var stack = (e.stack || '');
      ...
      stack = stack.split ('\n');
      e.luaStack = [stack[0], stack[1]];
    }

    if (!e.luaStack) e.luaStack = [];
    e.luaStack.push ('at ' + (this._data.sourceName || 'function') + ' on line ' + this._data.linePositions[this._pc - 1]);

    if (!e.luaCallStack) e.luaCallStack = [null];
    e.luaCallStack.push (this);
    if (luajs.debug) luajs.debug.showErrorStack (e.luaCallStack);
...
luajs.Closure.prototype._run = function () {
  ...
  // TODO: Does having these as separate if statements affect the execution?
...
/**
 * Retrieves the corresponding Lua file, if exists.
 */
luajs.File.prototype.loadLua = function () {
  var me = this,
    url = this._url.replace (/.json$/, '.lua');
  
  jQuery.ajax ({
    url: url, 
    success: function (data) {
      me.lua = data;
      me._trigger ('lua-loaded', data);
    },
    error: function () {
      me._trigger ('lua-load-falied');
    }
  });
};
...
//console.log ('print>>', item);
  ...
  setmetatable: function (table, metatable) {
    ...
    if (!(metatable instanceof luajs.Table) && metatable !== undefined) throw new luajs.Error ('Bad argument #2 in setmetatable(). Table expected');  
